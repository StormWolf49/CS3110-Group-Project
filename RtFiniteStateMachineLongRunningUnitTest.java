...
class RtFiniteStateMachineLongRunningUnitTest {
    ...
    void testS() {
        String word = "dcccd";
        FiniteStateMachine machine = this.buildTestStateMachine();
    ...
    private FiniteStateMachine buildTestStateMachine() {
        ArrayList<State> states = new ArrayList<State>();
        states.add(new RtState());
        states.add(new RtState(true));

        states.get(0).with(new RtTransition("c", states.get(0)));
        states.get(0).with(new RtTransition("d", states.get(1)));
        states.get(1).with(new RtTransition("c", states.get(0)));
        states.get(1).with(new RtTransition("d", states.get(1)));
        return new RtFiniteStateMachine(states.get(0));
    }
}
